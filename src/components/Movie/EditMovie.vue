      ...
      <table class="table">
        <tbody>
          ...
          <tr>
            ...
            <td>
              <input type="text" v-model="movie.length" disabled />
            </td>
          </tr>
...
export default {
  ...
  methods: {
    ...mapActions("movie", ["showInfo", "clearInfo", "setInfo"]),
    validateAndUpdate: function() {
      ...
      metadata.length = this.movie.length;
      // files
      let files = {};
    ...
    fileHandle: function() {
      ...
      getBlobDuration(fileURL).then(duration => {
        this.movie.length = parseInt(duration)
      });
    }
  },
  created() {
